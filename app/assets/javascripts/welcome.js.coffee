...
$ ->
  ...
  $("div.bord").delegate "div.point", "mouseenter", ->
    # class likes "position x-0 y-1"
    classes = $(this).attr('class').split(' ')
    x = classes[1].split('-')[1]
    y = classes[2].split('-')[1]

    removeHighlight(item) for item in $("div.point")

    addHighlightX(item) for item in $("div.point.x-#{x}")
    addHighlightY(item) for item in $("div.point.y-#{y}")
